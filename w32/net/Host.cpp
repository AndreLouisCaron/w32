...
namespace {
    ...
    const ::hostent * fetch ( const char * name )
    {
        ...
        // Note: type is address family (AF_INET, AF_INIT6, etc.).
    const ::hostent * fetch ( const void * address, int length, int type )
    {
            // Note: static entry allocated in thread-local storage.
        struct ::hostent * entry = ::gethostbyaddr
            (reinterpret_cast<const char*>(address), length, type);
        if ( entry == 0 ) {
...
namespace w32 { namespace net {
    ...
    Host::Host ( const ipv4::Address& address )
    {
        const ::hostent * data =
            ::fetch(&address.value(), sizeof(address.value()), AF_INET);
        ::memcpy(&myData, data, sizeof(myData));
    }

    Host::Host ( const ipv6::Address& address )
    {
        const ::hostent * data =
            ::fetch(&address.value(), sizeof(address.value()), AF_INET6);
        ::memcpy(&myData, data, sizeof(myData));
    }
